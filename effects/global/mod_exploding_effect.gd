...
# ============================================================
# 效果基类：本 mod 效果通用基类（ExplodingEffect 分支）
#   基类已自持久化 chance / explosion_scene / scale /
#   base_smoke_amount / sound_db_mod，这里只补自定义字段持久化。
# ------------------------------------------------------------
# 约定：
...
# 先还原基类字段（含 explosion_pool_id 重建），再还原自定义字段与白名单非导出变量
