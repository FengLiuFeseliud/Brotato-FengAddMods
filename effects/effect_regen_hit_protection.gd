...
func apply(player_index: int) -> void:
	FengLiuUtils.bind_effect(custom_key_hash, player_index, [key_hash, value, wait_time, gain_value])
	

func unapply(player_index: int) -> void:
	FengLiuUtils.unbind_effect(custom_key_hash, player_index, [key_hash, value, wait_time, gain_value])
...
func get_args(player_index: int) -> Array:
	...
	#   [2] = 倍率属性图标文本
	return [
        FengLiuUtils.text_percent(FengLiuUtils.get_dynamic_chance(value, gain_value, int(Utils.get_stat(key_hash, player_index)))),
        str(wait_time),
