...
func fengliu_set_items_price_from_shop_item_count(price: int, player_index: int) -> void:
	...
	if not effects.has(FengLiuKeys.fengliu_shop_items_count_price()):
		effects[FengLiuKeys.fengliu_shop_items_count_price()] = 0

	effects[Keys.items_price_hash] = effects[Keys.items_price_hash] + abs(effects[FengLiuKeys.fengliu_shop_items_count_price()]) + new_price
	effects[FengLiuKeys.fengliu_shop_items_count_price()] = new_price
...
func fill_shop_items(player_locked_items: Array, player_index: int, just_entered_shop: bool = false) -> void:
	...
	if effect[3] == 0:
		...
		var new_shop_item_count = ItemService.NB_SHOP_ITEMS - FengLiuUtils.get_dynamic_value(effect[2], effect[3], effect[4])
...
func fengliu_add_item_bought_elite(effect: Array, shop_item: ShopItem, player_index: int) -> void:
	...
	if effect[2] == 0:
		...
		chance = (effect[1] + FengLiuUtils.count_player_items(shop_item.item_data.my_id_hash, player_index) * (effect[2] / 100.0)) / 100.0
...
func on_shop_item_bought(shop_item: ShopItem, player_index: int) -> void :
	# 购买指定道具生成精英
	for effect in RunData.get_player_effect(FengLiuKeys.effect_fengliu_item_bought_spawn_boss(), player_index):
		if shop_item.item_data.my_id_hash == effect[0]:
			fengliu_add_item_bought_elite(effect, shop_item, player_index)

	var temp_stop_count = RunData.get_player_effect(FengLiuKeys.effect_fengliu_temporary_stats_stop(), player_index)
	if temp_stop_count is int and temp_stop_count > 0:
		for effect in RunData.get_player_effect(FengLiuKeys.effect_fengliu_stats_buy_item(), player_index):
		...
		player_effects[FengLiuKeys.effect_fengliu_temporary_stats_stop()] -= 1
