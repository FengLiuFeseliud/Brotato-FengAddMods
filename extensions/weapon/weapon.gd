...
func fengliu_gain_random_killed_stat(effect: NullEffect) -> void:
	...
	# 随机取值并添加属性
	var random_val = FengLiuUtils.get_dynamic_value(effect.stat_min_value, effect.stat_max_value, effect.stat_no_zero)
...
func fengliu_weapon_killed_loot(effect: NullEffect, spawn_pos: Vector2) -> void:
	...
	# 生成箱子
	FengLiuUtils.spawn_consumable(null, spawn_pos, 200, 500, Tier.UNCOMMON)
...
func on_weapon_hit_something(thing_hit: Node, damage_dealt: int, hitbox: Hitbox) -> void :
	...
	for effect in effects:
		if effect.custom_key_hash == FengLiuKeys.effect_fengliu_weapon_hit_slow():
...
func on_killed_something(_thing_killed: Node, hitbox: Hitbox) -> void :
	...
	# 遍历武器效果
	for effect in effects:
		if effect.custom_key_hash == FengLiuKeys.effect_fengliu_gain_random_killed_stat():
			# 杀敌获取随机值主要属性
			fengliu_gain_random_killed_stat(effect)
			continue

		if effect.custom_key_hash == FengLiuKeys.effect_fengliu_weapon_killed_loot():
			# 杀敌获取箱子
			fengliu_weapon_killed_loot(effect, _thing_killed.global_position)
			continue

		if effect.custom_key_hash == FengLiuKeys.effect_fengliu_weapon_killed_health():
			# 杀敌回复生命
			fengliu_weapon_killed_health(effect)
			continue

		# 杀敌添加临时属性
		if effect.custom_key_hash == FengLiuKeys.effect_fengliu_wpapon_killed_add_temp_stat():
